...
export default function GalleryCard({ creation }: GalleryCardProps) {
  return (
    ...
    >
      {creation.status === "nouveau" && <Badge className="absolute top-2 left-2 z-10 bg-green-500">Nouveau</Badge>}
      {creation.status === "vedette" && <Badge className="absolute top-2 left-2 z-10 bg-creasoka">Vedette</Badge>}
      {creation.status === "adopté" && <Badge className="absolute top-2 left-2 z-10 bg-red-500">Adopté</Badge>}
